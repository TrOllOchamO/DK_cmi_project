...
{
public:
    Player(float x, float y, int width, int height, float rotation = 0, bool hasGravity = false, float velocityOnX = 0, float velocityOnY = 0, bool collide = true, sf::Color color = sf::Color::White, sf::Texture texture = sf::Texture());
    ~Player();
    ...
    bool update_player(Inputs &inputs, float dt, const sf::Image img[]); 
